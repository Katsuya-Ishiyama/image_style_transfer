...
def get_commandline_args():
    ...
    parser.add_argument(
        '--content-file',
        help='Image file to be converted its style.',
        type=str,
        required=True
    )
    parser.add_argument(
        '--style-file',
        help='Image file that is the reference of style of converting images.',
        type=str,
        required=True
    )
    parser.add_argument(
        '--output-file',
        help='Resulting image file.',
        type=str,
        required=True
    )
...
if __name__ == '__main__':
    ...
    content_src = read_images_as_jpeg(args.content_file)
    style_src = read_images_as_jpeg(args.style_file)
    ...
    for i in range(args.train_steps):
        synthesized_image_val, _ = sess.run([synthesized_image, train_step],
                 feed_dict={
        ...
        if (i + 1) % args.eval_steps == 0:
            ...
            synthesize_val  = sess.run(
                    [synthesized_style_feature],
                     feed_dict={
                         content_feature: content_feature_map,
                         style1: style_feature_map[0],
                         style2: style_feature_map[1],
    ...
    result = unprocess(synthesized_image_val, mean_pixel)[0].astype(np.uint8)
    # tensorflowのIOを使わないとGoogle Storageに出力できない
    with file_io.FileIO(args.output_file, 'w') as f:
        Image.fromarray(result).save(f, quality=95)
